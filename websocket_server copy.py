...
CHECK_INTERVAL = 0.1  # How often to check for silence (seconds)
...
class WebSocketServer:
        ...
        try:
            ...
            if rms > SILENCE_THRESHOLD:
                ...
                if not conn_state["is_speaking"]:
                    logger.debug(f"Speech started for {connection_id}")
    ...
    async def generate_llm_response(self, websocket, transcription, connection_id):
        ...
        conversation.append({"role": "user", "content": transcription}) # Add user message
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(LLM_ENDPOINT, json={
                    ...
                    logger.info(f"LLM Response for {connection_id}: {complete_response}")
