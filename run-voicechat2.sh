...
# path to your llm
LLM_MODEL=llama.cpp/Meta-Llama-3-8B-Instruct-Q4_K_M.gguf 
...
create_window() {
    ...
    tmux send-keys -t "$window_name" "$command" C-m
}

# Create a new tmux session named 'voicechat2' or attach to it if it already exists
tmux new-session -d -s voicechat2

# FastAPI server (with conda activation)
create_window "voicechat2" "conda activate voicechat2 && uvicorn voicechat2:app --host 0.0.0.0 --port 8000 --reload"

# SRT server (HF transformers w/ distil-whisper)
create_window "voicechat2" "conda activate voicechat2 && uvicorn srt-server:app --host 0.0.0.0 --port 8001 --reload"

# LLM server (llama.cpp)
create_window "llm" "llama.cpp/build/bin/llama-server --host 127.0.0.1 --port 8002 -m $LLM_MODEL -ngl 99 -c $LLM_CONTEXT"

# TTS server (with conda activation)
create_window "tts" "conda activate voicechat2 && uvicorn tts-server:app --host 0.0.0.0 --port 8003"
